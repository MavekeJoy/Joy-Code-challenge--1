...
function calculateTax(income){
    ...
    //define our tax page on the bases of the tax rates
    const taxPage= [
    ...
    //iterate theough each tax page calculate the tax 
    for (const page of taxPage){
        //check if there is any remaining income to be taxed
        if(remainIncome <=0) break;
        //calculate taxable amount within the current page
        const taxableAmount =Math.min(remainIncome, page.limit);
        //calculate the taxfor the taxable amount
        tax+= taxableAmount *page.rate;
